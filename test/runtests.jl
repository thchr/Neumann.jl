...
@testset "Neumann.jl" begin
    @testset "Centrosymmetry" begin
        ...
        for N in 1:7
            rels = neumann_relations(inversion, N)
            if iseven(N)
    ...
    @testset "Kleinmann" begin
        ...
        for D in 1:4
            rels₂ = neumann_relations(I(D), 2, kleinmann=true)
        ...
        rels₃ = neumann_relations(I(3), 3, kleinmann=true)
        @test length(rels₃) == 18
    end
    ...
    @testset "README examples" begin
        ...
        rels_C₄ = neumann_relations(ops_C₄, 3)
        ...
        # C₄ + Kleinmann
        klein_rels_C₄ = neumann_relations(ops_C₄, 3; kleinmann=true)
    ...
    @testset "Miscellaneous" begin
        ...
        @test (neumann_relations(ops, 3) == neumann_relations(Tuple(ops), 3)
                                         == neumann_relations((op for op in ops), 3)
                                         == neumann_relations((@view ops[1:end]), 3))

        # span identical column space with/without sparsification
        for N in 2:6
            A = Neumann.neumann(ops, Val(N), Val(3); sparsify=true)
            B = Neumann.neumann(ops, Val(N), Val(3); sparsify=false)
        ...
        @test (neumann_relations(ops, 3; rref_tol=1e-12, atol=1e-11) == 
               neumann_relations(ops, 3; rref_tol=1e-5, atol=1e-4))

        # check that sparsification succeeds, even for quite large tensor orders
        for N in 1:7
            rels = neumann_relations(ops, N)
    ...
    @testset "Error handling" begin
        @test_throws ErrorException neumann_relations(I(3), 3; atol=1e-6, rref_tol=1e-5)
        @test_throws ErrorException neumann_relations([I(3), [0 1; 1 0]], 3)
    end
end
